...
2. An agent can be unlinked from a queue regardless of its current status.
3. The same agent cannot be linked to the same queue more than once.
4. Agent–queue associations are explicitly created and removed.

## Notes
- When an agent is linked to a queue, not mean that the agent is ready to receive conversations.
